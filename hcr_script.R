# This R Script is to view all pollutants data for Hounslow's Cranford (HCR)
...
comb_df_hcr$date_time_stamp <- combine.date.and.time(date = date_obj, time = time_obj)
...
comb_df_hcr$hcr_pm10 <- format(round(comb_df_hcr$hcr_pm10, 2), nsmall = 3)
comb_df_hcr$hcr_no <- format(round(comb_df_hcr$hcr_no, 2), nsmall = 3)
comb_df_hcr$hcr_no2 <- format(round(comb_df_hcr$hcr_no2, 2), nsmall = 3)
comb_df_hcr$hcr_noxasno2 <- format(round(comb_df_hcr$hcr_noxasno2, 2), nsmall = 3)
comb_df_hcr$hcr_nv10 <- format(round(comb_df_hcr$hcr_nv10, 2), nsmall = 3)
comb_df_hcr$hcr_o3 <- format(round(comb_df_hcr$hcr_o3, 2), nsmall = 3)
comb_df_hcr$hcr_so2 <- format(round(comb_df_hcr$hcr_so2, 2), nsmall = 3)
comb_df_hcr$hcr_v10 <- format(round(comb_df_hcr$hcr_v10, 2), nsmall = 3)

#Calling all col titles
colnames(comb_df_hcr)

# Re-arrangement of data including removal of end_date & end_time from the data set
comb2_df_hcr <- comb_df_hcr %>% 
  select(11,3:10)
...
# Step 5: hcr Data Export (VCM)
# ===============================

df_spot_hcr <- readr::write_csv(comb2_df_hcr, path = "/dsq_data_projects/aq_project/EH_project_work/datasets/filter_by_hotspots/comb_df_hcr_with_decimals.csv")
